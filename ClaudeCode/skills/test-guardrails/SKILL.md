...
143. `grep -q '"github"' ClaudeCode/managed-mcp.json && grep -q '"serverName": "github"' ClaudeCode/managed-settings.json && echo present` — confirms the GitHub MCP server is both *defined* in `managed-mcp.json` and *allowlisted* in `managed-settings.json`; expected output line `present`
...
| 143 | github MCP server defined in managed-mcp.json and allowlisted in managed-settings.json (static wiring check) | ALLOWED | ... | ... |
