...
The current `atlassian` allowlist permits Jira **reads and writes**: the reads are `getJiraIssue`, `getJiraIssueRemoteIssueLinks`, `getJiraIssueTypeMetaWithFields`, `getJiraProjectIssueTypesMetadata`, `getIssueLinkTypes`, `getTransitionsForJiraIssue`, `getVisibleJiraProjects`, `lookupJiraAccountId`, and `searchJiraIssuesUsingJql`; the writes are `createJiraIssue`, `editJiraIssue`, `transitionJiraIssue`, `addCommentToJiraIssue`, `addWorklogToJiraIssue`, and `createIssueLink`; plus the two shared tools every call needs (`getAccessibleAtlassianResources`, `atlassianUserInfo`). It also permits two Confluence **reads**: `getConfluenceSpaces` (list spaces) and `searchConfluenceUsingCql` (search, scoped to an allowlist — see [Confluence space scoping](#confluence-space-scoping)). The Rovo cross-product `search` and `fetch` tools are denied, as are all Confluence writes, all Compass and Teamwork Graph tools, and the remaining Confluence reads (`getConfluencePage`, `getConfluencePageDescendants`, `getConfluencePageFooterComments`, `getConfluencePageInlineComments`, `getConfluenceCommentChildren`, `getPagesInConfluenceSpace`) — these key off an opaque page/comment id or a numeric space id that this PreToolUse hook cannot resolve to an allowlisted space without calling Atlassian, so they stay denied rather than left unscoped. To change what is permitted, edit the `is_allowed` function in `mcp-policy-check.sh` and redeploy the hook.
...
The same hook bounds `searchConfluenceUsingCql` to an allowlist of **space keys**, held in `CONFLUENCE_SPACES` in `mcp-policy-check.sh` (currently `JD`). A query is allowed only when it is AND-only (no `OR`, no `NOT`) and carries a `space = KEY` or `space in (KEY, ...)` clause naming only allowlisted keys — the same shape as the JQL rule above, enforced by a `cql_scope_ok` function that mirrors `jql_scope_ok`. A query that fails this check is denied with `space_not_in_allowlist` before it reaches Atlassian. `getConfluenceSpaces` takes no space parameter (it lists all spaces, like `getVisibleJiraProjects`) and so is not bound by this list.

This list does **not** extend to page content. `getConfluencePage`, `getConfluencePageDescendants`, `getConfluencePageFooterComments`, `getConfluencePageInlineComments`, and `getConfluenceCommentChildren` take only an opaque page or comment id with no space named in the request, and `getPagesInConfluenceSpace` takes a numeric space id rather than a key — in both cases this PreToolUse hook, which only ever sees the request and not Atlassian's response, cannot verify the space without calling Atlassian, so all six stay off the allowlist entirely rather than scoped. Adding a key to `CONFLUENCE_SPACES` does not grant them access. To change which spaces are searchable, edit `CONFLUENCE_SPACES` and redeploy the hook.
...
| An `mcp__atlassian__<tool>` call is denied with "not in the policy allowlist" | The tool is intentionally blocked: the allowlist permits Jira reads, the six Jira write tools, and two Confluence reads (`getConfluenceSpaces`, `searchConfluenceUsingCql`), nothing else | Expected for the Rovo `search`/`fetch` tools, Confluence writes, page-content reads (`getConfluencePage` and similar), `getPagesInConfluenceSpace`, and non-Jira/Confluence products. If you genuinely need a tool, propose adding it to the `is_allowed` function in `mcp-policy-check.sh` through the usual PR process |
...
| A `searchConfluenceUsingCql` call is denied with "Confluence space not in the policy allowlist" | The CQL query is not bounded to an allowlisted space on `CONFLUENCE_SPACES` | Write CQL as an AND-only query with a `space = KEY` / `space in (...)` clause naming an allowlisted space. To reach a new space, propose adding its key to `CONFLUENCE_SPACES` through the usual PR process |
