...
- **Subcommand allowlist**: only `gh issue|pr|repo|gist|label|release` pass the hook at all; `gh auth` (including `gh auth token`, which prints the live credential), `gh api`, `gh secret` and everything else are denied by default. On top of that, the `gh_subcommand` pre-block denies `gist create/edit`, `repo delete/archive/rename/edit/create/fork`, `release create/upload/delete/edit`, and `pr merge --admin`.
...
The current `atlassian` allowlist permits Jira **reads and writes**: the reads are `getJiraIssue`, `getJiraIssueRemoteIssueLinks`, `getJiraIssueTypeMetaWithFields`, `getJiraProjectIssueTypesMetadata`, `getIssueLinkTypes`, `getTransitionsForJiraIssue`, `getVisibleJiraProjects`, `lookupJiraAccountId`, and `searchJiraIssuesUsingJql`; the writes are `createJiraIssue`, `editJiraIssue`, `transitionJiraIssue`, `addCommentToJiraIssue`, `addWorklogToJiraIssue`, and `createIssueLink`; plus the two shared tools every call needs (`getAccessibleAtlassianResources`, `atlassianUserInfo`). The Rovo cross-product `search` and `fetch` tools are denied, as are all Confluence, Compass, and Teamwork Graph tools. To change what is permitted, edit the `is_allowed` function in `mcp-policy-check.sh` and redeploy the hook.
